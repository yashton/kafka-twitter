...
object KafkaStreamImplicits {
  ...
  {
    def streamS[K, V](topic: String)
      (implicit ks: Serde[K], vs: Serde[V]): KStream[K, V] =
    {
  ...
  {
    def sendTo(topic: String)
      (implicit ks: Serde[K], vs: Serde[V]): Unit =
    {
  ...
  implicit class KGroupedStreamWrapper[K, V](stream: KGroupedStream[K, V]) {
    def agg[U](init: Initializer[U], agg: Aggregator[K, V, U], store: String)
      (implicit s: Serde[U]): KTable[K, U] =
    {
